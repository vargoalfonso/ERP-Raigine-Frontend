...
function IndirectRawMaterialDetailPageContent() {
  ...
  const detailQuery = useGetInventoryDetailQuery({ type: "indirect-materials", id }, { skip: !apiEnabled || !id });
  const incomingQuery = useGetInventoryIncomingQuery({ type: "indirect-materials", page: 1, limit: 20 }, { skip: !apiEnabled });
  ...
  useEffect(() => {
    ...
    if (isMissingRouteError(error)) {
      message.warning("Inventory indirect-materials API route is not available yet; showing placeholder data.");
      return;
    }
    message.error(getApiErrorMessage(error, "Failed to load indirect raw material detail"));
        ...
        <Card className="rounded-2xl shadow" loading={apiEnabled ? detailQuery.isFetching || summaryQuery.isFetching : false}>
          ...
          <Tabs
            ...
            items={[
              ...
              {
                key: "logs",
                label: <span className="flex items-center gap-2">🧾 Incoming Logs</span>,
                children: (
                  <div className="mt-6">
                    <div className="flex items-start justify-between gap-4 flex-wrap">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <Card className="rounded-xl" styles={{ body: { padding: 16 } }}>
                          <div className="text-gray-500 text-sm">Total Incoming Logs</div>
