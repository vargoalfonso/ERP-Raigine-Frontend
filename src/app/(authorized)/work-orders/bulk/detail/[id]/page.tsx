...
import { useGetWorkOrderItemQRQuery } from "@/lib/api/work-orders/api";
import { useGetBulkWorkOrderByIdQuery } from "@/lib/api/work-orders/bulk/api";
import {
...
export default function BulkWorkOrderDetailPage() {
  ...
  const { data: workOrder, isFetching } = useGetBulkWorkOrderByIdQuery(
    id ?? "",
    {
      skip: !apiEnabled,
    },
        ...
        <header className="mb-5 flex min-w-0 flex-col gap-4 border-b border-slate-200 pb-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0">
            ...
            <h1 className="min-w-0 break-words text-2xl font-semibold tracking-[-0.02em] text-slate-950 sm:text-3xl">
              Bulk work order detail
            </h1>
            <p className="mt-1 text-sm text-slate-500">
              Review the bulk work order and inspect QR details for each UNIQ
              item.
            </p>
          </div>
              ...
              <div className="min-w-0 p-4 sm:p-5">
                <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    >
                      Bulk work order overview
                    </h2>
                    <p className="mt-1 text-xs text-slate-500">
                      Current bulk order status and production context.
                    </p>
                  </div>
                  <Tag
                ...
                {(workOrder?.defect_reasons?.length ?? 0) > 0 ||
                workOrder?.defect_reason ? (
                  <div className="mt-4 rounded-md border border-amber-200 bg-amber-50 p-3">
                    ...
                    {(workOrder?.defect_reasons?.length ?? 0) > 0 ? (
                      <Tooltip
                        title={
                          <div className="space-y-1">
                            {workOrder!.defect_reasons!.map((r, i) => (
                              <div key={i}>
                                {r.source === "scrap_reason" ? "Scrap" : "NG"}:{" "}
                                {r.info || "-"} (Qty: {r.qty})
                              </div>
                            ))}
                          </div>
                      ...
                      >
                        <div className="mt-1 inline-flex cursor-help items-center gap-1 break-words text-sm font-semibold text-amber-900 underline decoration-dotted underline-offset-2">
                          {workOrder!.defect_reasons![0].info ||
                            workOrder?.defect_reason ||
                            "-"}
                          {workOrder!.defect_reasons!.length > 1
                            ? ` +${workOrder!.defect_reasons!.length - 1} lainnya`
                            : ""}
                        </div>
                      </Tooltip>
                    ) : (
