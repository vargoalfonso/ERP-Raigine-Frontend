...
export default function ApprovalManagerPage() {
  ...
  const [statusFilter, setStatusFilter] =
    useState<ApprovalStatusFilter>("All Status");

  const [selectedItem, setSelectedItem] = useState<ApprovalManagerItem | null>(
    null,
  );
  const [remarks, setRemarks] = useState<string>("");
  const [selectedRowKeys, setSelectedRowKeys] = useState<string[]>([]);
  const [bulkActionLoading, setBulkActionLoading] =
    useState<ApprovalManagerDecision | null>(null);
  ...
  const [fetchDetail, fetchDetailState] =
    useLazyGetApprovalManagerDetailByUrlQuery();
  const [submitDecision, submitDecisionState] =
    useSubmitApprovalManagerDecisionMutation();
  ...
  const summaryQuery = useGetApprovalManagerSummaryQuery(
    { type },
    { skip: !apiEnabled },
  );
  ...
  type ModuleKind = "stock_opname" | "bom" | "prl" | "po_budget" | "wo" | "unknown";
  const getModuleKind = (item: ApprovalManagerItem | undefined): ModuleKind => {
    const label = String(
      item?.module_label ?? item?.module ?? "",
    ).toLowerCase();
  ...
  const rows = useMemo<ApprovalRow[]>(() => {
    ...
    const items = (listQuery.data?.data ?? []).filter(
      (it) => it.can_view === true,
    );
    return items.map((it) => {
      const id = String(
        it.document_id ?? it.item_code ?? it.reference_id ?? it.instance_id,
      );
      ...
      return {
        ...
        submittedBy:
          String(it.submitted_by_name ?? it.submitted_by ?? "-") || "-",
  ...
  useEffect(() => {
    setSelectedRowKeys((prev) =>
      prev.filter((key) => filteredRows.some((row) => row.key === key)),
    );
  }, [filteredRows]);
  ...
  const onView = async (item: ApprovalManagerItem | undefined) => {
    ...
    const title =
      String(
        item.item_name ?? item.item_code ?? item.document_id ?? "Detail",
      ).trim() || "Detail";
  ...
  const onApprove = async (
    ...
    try {
      ...
      messageApi.error(
        getApiErrorMessage(
          error,
          action === "approve" ? "Failed to approve" : "Failed to reject",
        ),
      );
    }
  };
  ...
  const handleBulkDecision = async (action: ApprovalManagerDecision) => {
    const candidates =
      action === "approve" ? selectedApprovableRows : selectedRejectableRows;
    ...
    try {
      for (const row of candidates) {
        try {
          ...
          failures.push(
            `${row.itemCode}: ${getApiErrorMessage(error, `Failed to ${action}`)}`,
          );
        }
      }
  ...
  const renderDetailBody = () => {
    ...
    const submittedBy =
      String(item?.submitted_by_name ?? item?.submitted_by ?? "-") || "-";
    ...
    const renderStockOpname = () => {
      ...
      const session = isRecord(payload.session)
        ? (payload.session as UnknownRecord)
        : null;
      ...
      const entryColumns: ColumnsType<(typeof entryRows)[number]> = [
        ...
        },
        {
          ...
          width: 110,
          render: (v: unknown) => (
            <Tag className="!rounded-full">{toText(v)}</Tag>
          ),
        },
      ];
    ...
    const renderBom = () => {
      ...
      const obj =
        rawObj && isRecord(rawObj.data)
          ? (rawObj.data as UnknownRecord)
          : rawObj;
      if (!obj) return missing;

      const processRoutes = Array.isArray(obj.process_routes)
        ? (obj.process_routes as unknown[])
        : [];
      ...
      const material = isRecord(obj.material_spec)
        ? (obj.material_spec as UnknownRecord)
        : null;
      ...
      const renderBomNode = (
        ...
        const nodeMaterial = isRecord(node.material_spec)
          ? (node.material_spec as UnknownRecord)
          : null;
        const nodeAsset = isRecord(node.asset)
          ? (node.asset as UnknownRecord)
          : null;
        ...
        const nodeProcessRows = nodeProcessRoutes
          ...
          .map((r) => ({
            key: String(
              toText(r.route_id ?? r.op_seq ?? `${numbering}-${Math.random()}`),
            ),
            op_seq: toNumber(r.op_seq) ?? 0,
            process_name: toText(r.process_name),
            machine_name: toText(r.machine_name),
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div>
                <div className="text-sm font-semibold text-gray-900">
                  {depth === 0
                    ? "Root BOM"
                    : `Child Level ${toText(node.level) !== "-" ? toText(node.level) : depth}`}
                  {numbering ? ` • ${numbering}` : ""}
                </div>
              ...
              <div className="mt-4">
                ...
                <div className="mt-2 grid grid-cols-1 gap-3 md:grid-cols-2">
                  ...
                  {kv(
                    "Supplier",
                    toText(
                      nodeMaterial.supplier_name ?? nodeMaterial.supplier_id,
                    ),
                  )}
              ...
              <div className="mt-4 space-y-3">
                ...
                {childNodes.map((child, index) =>
                  renderBomNode(
                    child,
                    depth + 1,
                    numbering ? `${numbering}.${index + 1}` : `${index + 1}`,
                  ),
                )}
              </div>
            ) : null}
      ...
      return (
        <>
          ...
          {material ? (
            <div className="mt-4">
              ...
              <div className="mt-2 grid grid-cols-1 gap-3 md:grid-cols-2">
                ...
                {kv(
                  "Supplier",
                  toText(material.supplier_name ?? material.supplier_id),
                )}
          ...
          {childNodes.length > 0 ? (
            <div className="mt-4">
              <div className="text-sm font-semibold text-gray-900">
                BOM Structure / Children
              </div>
              <div className="mt-2 space-y-3">
                {childNodes.map((child, index) =>
                  renderBomNode(child, 1, `${index + 1}`),
                )}
              </div>
            </div>
          ) : null}
    ...
    const renderPrl = () => {
      if (!isRecord(payload)) return missing;
      const prlObj = isRecord(payload.prl)
        ? (payload.prl as UnknownRecord)
        : null;
    ...
    const renderPoBudget = () => {
      ...
      const { basic, calc, result, additional, history } =
        normalizePoBudgetPayload(payload);
      ...
      const historyColumns: ColumnsType<(typeof historyRows)[number]> = [
        ...
        },
        {
          ...
          render: (value: string) => {
            ...
            const color = lower.includes("approve")
              ? "green"
        ...
        <>
          <div className="mt-4">
            ...
            <div className="mt-2 grid grid-cols-1 gap-3 md:grid-cols-2">
              {kv(
                "PO Budget Ref",
                toText(basic.po_budget_ref ?? basic.poBudgetRef ?? basic.id),
              )}
              ...
              {kv(
                "Budget Type",
                toText(
                  basic.type_label ?? basic.budget_type ?? basic.budget_subtype,
                ),
              )}
              ...
              {kv(
                "Status",
                toText(
                  additional.status ?? result.apo_prl_state ?? basic.status,
                ),
              )}
            </div>
          </div>
          ...
          <div className="mt-4">
            ...
            <div className="mt-2 grid grid-cols-1 gap-3 md:grid-cols-2">
              ...
              {kv(
                "APO vs PRL",
                formatNumber(result.apo_prl_abs ?? result.apo_prl),
              )}
          ...
          <div className="mt-4">
            ...
            <div className="mt-2 grid grid-cols-1 gap-3 md:grid-cols-2">
              {kv(
                "Submitted By",
                toText(additional.submitted_by_name ?? additional.submitted_by),
              )}
              {kv("Submitted At", toText(additional.submitted_at))}
              {kv(
                "Approved By",
                toText(additional.approved_by_name ?? additional.approved_by),
              )}
              {kv(
                "Approved At",
                toText(additional.approved_at ?? additional.approval_date),
              )}
              {kv(
                "Notes",
                toText(
                  additional.notes ?? additional.note ?? additional.remarks,
                ),
              )}
            </div>
          </div>
  ...
  const columns: ColumnsType<ApprovalRow> = [
    ...
    },
    {
      ...
      render: (value: string) => {
        const klass =
          moduleTagClass[value] ?? "bg-gray-50 text-gray-700 border-gray-200";
        return (
          <Tag
            className={`rounded-full border px-2 py-0.5 text-xs font-semibold ${klass}`}
          >
            {value}
          </Tag>
        );
    ...
    },
    {
      ...
      render: (value: ApprovalStatus) => (
        <Tag
          className={`rounded-full border px-2 py-0.5 text-xs font-semibold ${statusTagClass[value]}`}
        >
          {value}
        </Tag>
      ),
            ...
            <>
              {row.backend?.can_approve === true ? (
                <Button
                  ...
                  disabled={
                    !canActionable(row.backend) || submitDecisionState.isLoading
                  }
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="mt-1 text-gray-500">
              Centralized approval workflow for all modules requiring manager
              authorization
            </p>
          </div>
        ...
        <div className="mt-6">
          ...
          <Modal
            ...
            footer={(() => {
              ...
              const disabled =
                !canActionable(item ?? undefined) ||
                submitDecisionState.isLoading;
                ...
                <div className="flex items-center justify-end gap-2">
                  ...
                  {showReject ? (
                    <Button
                      ...
                      className="!border-red-200 !bg-red-50 !text-red-600 hover:!border-red-300 hover:!bg-red-100 hover:!text-red-700"
                      onClick={() =>
                        onApprove(item ?? undefined, "reject", remarks)
                      }
                      disabled={disabled}
                  ...
                  {showApprove ? (
                    <Button
                      ...
                      className="!border-emerald-600 !bg-emerald-600 hover:!border-emerald-700 hover:!bg-emerald-700"
                      onClick={() =>
                        onApprove(item ?? undefined, "approve", remarks)
                      }
                      disabled={disabled}
                      loading={submitDecisionState.isLoading}
                    >
          ...
          <Table<ApprovalRow>
            ...
            loading={
              apiEnabled && (listQuery.isFetching || summaryQuery.isFetching)
            }
            rowSelection={{
              fixed: true,
              selectedRowKeys,
