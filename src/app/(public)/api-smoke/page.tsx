...
const DEFAULT_ENDPOINTS: Array<{ method: string; path: string; requiresAuth?: boolean }> = [
  // Auth-required (most ERP-raigine routes)
  ...
  // Known-problem endpoint (still useful to verify backend)
...
export default function ApiSmokePage() {
  ...
  const [baseUrl, setBaseUrl] = useState<string>(apiBaseUrl || "http://localhost:3001");
  ...
  const run = async () => {
    ...
    for (let i = 0; i < next.length; i += 1) {
      const r = next[i];
      const url = `${base}${r.path.startsWith("/") ? "" : "/"}${r.path}`;
      ...
      try {
        ...
        // Read small body snippet for debugging
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <Paragraph>
        Pakai halaman ini untuk ngecek endpoint ERP-raigine dari browser. Cocok untuk test antar-local tanpa curl.
      </Paragraph>

      <Card style={{ marginBottom: 16 }}>
        <div className="grid gap-3" style={{ gridTemplateColumns: "160px 1fr" }}>
          <Text strong>Base URL</Text>
          <Input value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} placeholder="http://localhost:3001" />
